...
namespace SimpleMACross
{
    ...
    {
        [InputParameter("Telegam Chat ID")]
        public string TelegramID = "1111";

        [InputParameter("Telegam BOT token")]
        public string TelegramToken = "aaaaaaaaaa";
            ...
            if (Core.Instance.Positions.Length == 0)
            {
                ...
                if (this.indicatorFastMA.GetValue(1) > this.indicatorSlowMA.GetValue(1))
                {
                    ...
                    if (result.Status == TradingOperationResultStatus.Success)
                    {
                        ...
                        SendTelegramMessage(" Long position was placed.");
                    }
                ...
                else if (this.indicatorFastMA.GetValue(1) < this.indicatorSlowMA.GetValue(1))
                {
                    ...
                    if (result.Status == TradingOperationResultStatus.Success)
                    {
                        ...
                        SendTelegramMessage(" Short position was placed.");
                    }
        ...
        private void SendTelegramMessage(string txtMsg)
        {
            ...
            try
            {
                ...
                Core.Instance.Loggers.Log(TelegramURLString);

                Stream rs = request.GetResponse().GetResponseStream();
                StreamReader reader = new StreamReader(rs);
                string strResponse = reader.ReadToEnd();
                reader.Close();
                Core.Instance.Loggers.Log(strResponse);
            }
            catch
            {                
                Core.Instance.Loggers.Log("Error sending Telegram.");
            }
        }
    }
}
